...
async function rasterFromPreparation(preparation: ImagePreparationValue, width: number, height: number, sourceType: SourceType): Promise<RgbaRaster> {
  ...
  const sx = Math.round((crop.x / 100) * source.naturalWidth);
  const sy = Math.round((crop.y / 100) * source.naturalHeight);
  const sw = Math.max(1, Math.round((crop.width / 100) * source.naturalWidth));
  const sh = Math.max(1, Math.round((crop.height / 100) * source.naturalHeight));
  ...
  transformed.width = quarterTurn ? sh : sw; transformed.height = quarterTurn ? sw : sh;
  ...
  transformedContext.drawImage(cropped, -sw / 2, -sh / 2);
  ...
  const scale = preparation.fitMode === "cover" ? Math.max(width / transformed.width, height / transformed.height) : Math.min(width / transformed.width, height / transformed.height);
  const drawWidth = transformed.width * scale;
  const drawHeight = transformed.height * scale;
  const dx = preparation.fitMode === "contain" && preparation.paddingAlignment === "center" ? (width - drawWidth) / 2 : 0;
  const dy = preparation.fitMode === "contain" && preparation.paddingAlignment === "center" ? (height - drawHeight) / 2 : 0;
  context.clearRect(0, 0, width, height);
  context.drawImage(transformed, dx, dy, drawWidth, drawHeight);
